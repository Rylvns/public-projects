-- by Rylen
-- Last edited on Tuesday, April 9 (4/9/24); 7:33 PM (EST/EDT)
...
local function VerifyConfigs(Configs)
	...
	coroutine.wrap(function()
		...
		Signal = RunService.Heartbeat:Connect(function()
			if Parent ~= nil and Particle ~= nil then
				...
				Iteration += 1;
				if Iteration > 20 and IsOffScreen(Particle) == true then
					Particle:Destroy();
					Signal:Disconnect();
				end;
			else
...
function Module.New()
	if RunService:IsClient() == true then
		...
		function self:Emit(Configs)
			...
			task.spawn(VerifyConfigs, Configs);

			EmitParticle(Screen, Configs);
		end;
		return self;
	else
